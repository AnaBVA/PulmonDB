...
my_pca <- function(df,x= PC1, y = PC2, leg = 'none'){
  x <- enquo(x)
  y <- enquo(y)
...
#data_COPD <- n_data[,colSums(is.na(n_data)) <= s["3rd Qu."]]
data_COPD <- n_data

# Keep genes without NAs in all contrast
data_COPD <- data_COPD[rowSums(is.na(data_COPD)) == 0,] # A tibble: 13,627 x 164


anno_tissue <- gse %>%
  filter(contrast_name %in% colnames(data_COPD)) %>%
...
###########################
###PCA
e <- prcomp(t(data_COPD[,-1]))
rownames(e$x) <- colnames(data_COPD)[-1]
rownames(e$rotation) <- data_COPD$gene_name
...
var_exp <- fviz_screeplot(e,addlabels = TRUE, ylim = c(0,35), barfill = "#6b0778", barcolor ="white") +
  labs(title = "Variances in PCA components",x = "Principal Components", y = "% of variances") +
...
tpca_12 <- my_pca(df,PC1,PC2)
tpca_23 <- my_pca(df,PC2,PC3)

legend <- g_legend(my_pca(df,PC1,PC2,"right"))
...
tsne <- Rtsne(as.matrix(t(data_COPD[,-1])), dims = 3, check_duplicates = FALSE)

tsne_data <- data.frame(tsne$Y)
rownames(tsne_data) <- colnames(data_COPD)[-1]
...
ttsne <-  ggplot(data = tsne_data, aes(x=X1,y=X2,colour= test2),frame = TRUE,frame.type = 'norm') +
  #scale_color_manual(values = c("#930404", "#3f7ac6","#0dba79")) +
...
designDE <- model.matrix(~0 + DISEASE_STATUS + GSE ,colData(g))
